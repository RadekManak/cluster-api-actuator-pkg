	...
	When("machineset has one replica", framework.LabelDisruptive, Ordered, func() {
		...
		It("publish selector in the scale subresource for HPA targets", func() {
			...
			Eventually(func() error {
				...
				if current.Status.ObservedGeneration == nil {
					return fmt.Errorf("HPA has not observed generation %d yet", current.Generation)
				}

				if *current.Status.ObservedGeneration != current.Generation {
					return fmt.Errorf("expected observed generation %d, got %d", current.Generation, *current.Status.ObservedGeneration)
				}
